...
function App() {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      ...
      await getFormattedWeatherData({ ...query, units }).then((data) => {
        toast.success(
          `اطلاعات آب و هوایی ${data.name} دریافت شد.`
        );
  ...
  return (
    <div
      className={`mx-auto max-w-screen-md mt-4 py-5 px-32 bg-gradient-to-br rounded-xl h-fit shadow-xl shadow-gray-600 ${formatBackground()}`}
    >
